queries = {
    "OVERVIEW":("Project details", 
            '''MATCH (p:Project) 
                    ...
                    RETURN p.name AS Name, p.id AS Identifier, p.description AS Description, p.responsible AS Responsible, p.status AS Status, p.participants AS Participants, p.types AS Datasets;'''),
    "NUMBER_SUBJECTS":("Number of Enrolled Subjects:",
            '''MATCH (p:Project{id:"PROJECTID"})-[:HAS_ENROLLED]-(s:Subject) 
                    RETURN COUNT(DISTINCT(s)) AS Number_of_Subjects;'''),
    "NUMBER_ANALYTICAL_SAMPLES":("Number of Analytical Samples:", 
            '''MATCH (p:Project{id:"PROJECTID"})-[*3]-(a:Analytical_sample) 
                    RETURN COUNT(DISTINCT(a)) AS Number_of_Analytical_Samples;'''),
    "NUMBER_PROTEINS_ANALYTICAL_SAMPLE":("Number of Proteins", 
            '''MATCH p=(project:Project)-[*3]-(a:Analytical_sample)-[r:HAS_QUANTIFIED_PROTEIN]->(protein:Protein) 
                    ...
                    RETURN a.id AS name, a.id AS x,COUNT(DISTINCT(protein.id)) AS y, a.group AS group;'''),
    ...
    "IDENTIFIED_PROTEINS":("Identified proteins per group", 
            '''MATCH p=(project:Project)-[*3]-(a:Analytical_sample)-[r:HAS_QUANTIFIED_PROTEIN]->(protein:Protein) 
                    WHERE project.id="PROJECTID" 
                    RETURN protein.id AS identifier, toFloat(r.value) as LFQ_intensity;'''),
    "IDENTIFIED_PROTEINS_SAMPLE_GROUP":("Identified proteins per group", 
            '''MATCH p=(project:Project)-[*3]-(a:Analytical_sample)-[r:HAS_QUANTIFIED_PROTEIN]->(protein:Protein) 
                    WHERE project.id="PROJECTID" 
                    RETURN a.id AS sample, protein.id AS identifier, a.group AS group, toFloat(r.value) as LFQ_intensity;'''),
    ...
    "DRUG_ACTS_ON_PROTEINS":("Drug acts on protein",
            '''MATCH p=(project:Project)-[*3]-(a:Analytical_sample)-[r:HAS_QUANTIFIED_PROTEIN]->(protein:Protein)<-[i:ACTS_ON]-(drug:Drug) 
                    WHERE project.id="PROJECTID" AND protein.id IN [PROTEINS] AND i.action="ACTION" 
                    RETURN protein.id AS target, drug.id AS source, drug.name AS name, drug.description AS description, i.action AS action, toFloat(i.score) AS score;''')
}
